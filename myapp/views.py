    ...
    if request.method == 'POST':
        try:
            try:
                ...
                logger.error(f"JSON decode error in register: {jde}. Raw body: {request.body}")
                return JsonResponse({'error': 'Invalid JSON format or extra data in request body.'}, status=400)
            ...
            if not all([username, email, password, password2, role]):
                logger.warning(f"Register failed: Missing fields. Data received: {data}")
                return JsonResponse({'error': 'All fields are required.'}, status=400)

            if password != password2:
                logger.warning(f"Register failed: Passwords do not match. Data received: {data}")
                return JsonResponse({'error': 'Passwords do not match.'}, status=400)

            if User.objects.filter(username=username).exists():
                logger.warning(f"Register failed: Username already exists: {username}")
    ...
    if request.method == 'POST':
        try:
            try:
                ...
                logger.error(f"JSON decode error in google_login: {jde}. Raw body: {request.body}")
                return JsonResponse({'error': 'Invalid JSON format or extra data in request body.'}, status=400)
    ...
    if request.method == 'POST':
        try:
            try:
                ...
                logger.error(f"JSON decode error in login: {jde}. Raw body: {request.body}")
                return JsonResponse({'error': 'Invalid JSON format or extra data in request body.'}, status=400)
            ...
            try:
                ...
                if user is None:
                    logger.warning(f"Authentication failed for user found by email: {user_obj.username}")
            ...
            if user is not None:
                ...
                logger.info(f"Login successful for user: {user.username} with role: {role}")
                ...
                logger.debug(f"Failed login details - email: {email}, password length: {len(password) if password else 0}")
